...
try:
    ...
    # Kadang GitHub memberikan JSON
    # dalam bentuk string yang ter-encode.

    if (
        raw_payload.startswith('"')
        and raw_payload.endswith('"')
    ):
...
for id_rek in rekomendasi:
    ...
    <a
        href="{html.escape(rek_url, quote=True)}"
        class="rekomendasi-card"
    >

        <img
            src="{html.escape(rek_img, quote=True)}"
            class="rekomendasi-img"
            alt="{html.escape(rek_judul, quote=True)}"
        >

        <div class="rekomendasi-body">

            <span class="rekomendasi-date">
                {html.escape(str(rek_tanggal))}
            </span>

            <div class="rekomendasi-item-title">
                {html.escape(rek_judul)}
            </div>
...
<meta property="og:url"
   content="{html.escape(url_preview_artikel, quote=True)}">
...
```
    ...
    # PENTING:
    # Isi berita HTML TIDAK di-escape.
    # Supaya <p>, <strong>, <img>, dll
    # tetap berfungsi.
...
<meta name="description"
      content="Berita terbaru My Asrama">
...
<link>
    https://news.myasrama.my.id
</link>

<description>
    Preview portal berita resmi My Asrama
</description>
```
